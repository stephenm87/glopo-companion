...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (!imageBase64 || !mimeType) {
            return {
                statusCode: 400,
                body: JSON.stringify({ error: "File data missing." })
            };
        }
        ...
        if (!SUPPORTED_TYPES.includes(mimeType)) {
            return {
                statusCode: 400,
                body: JSON.stringify({ error: `Unsupported file type: ${mimeType}. Please upload an image or PDF.` })
            };
        }

        const genAI = new GoogleGenerativeAI(apiKey);
        // Use gemini-2.5-pro for highest quality IB examiner-level feedback (supports vision)
...
AO2 — APPLICATION & ANALYSIS: Named IR theory (Realism/Liberalism/Constructivism etc.) consistently applied with analytical connectives.
...
AO1: [Band 1/2/3] | AO2: [Band 1/2/3] | AO3: [Band 1/2/3] | AO4: [Band 1/2/3]`;
        ...
        try {
            ...
            if (genError.status === 429 || genError.message?.includes('429') || genError.message?.includes('quota')) {
                ...
                const retryResult = await model.generateContent([prompt, imagePart]);
